...
static int	add_color(char *line, int *color);
...
int	add_next_instruction(char *line, t_map_info *info_ptr)
{
	...
	{
		if (add_path(ft_strtrim(line + 3, " "), &info_ptr->path_no))
			return (1);
	}
	else if (!ft_strncmp("SO ", line, 3))
	{
		if (add_path(ft_strtrim(line + 3, " "), &info_ptr->path_so))
			return (1);
	}
	else if (!ft_strncmp("WE ", line, 3))
	{
		if (add_path(ft_strtrim(line + 3, " "), &info_ptr->path_we))
			return (1);
	}
	else if (!ft_strncmp("EA ", line, 3))
	{
		if (add_path(ft_strtrim(line + 3, " "), &info_ptr->path_ea))
			return (1);
	}
	else
...
static int	add_next_instruction_extention(char *line, t_map_info *info_ptr)
{
	if (!ft_strncmp("F ", line, 2) && !info_ptr->color_f_check)
	{
		if (add_color(line + 2, &info_ptr->color_f))
			return (1);
